...
from tabs.raw_cogs.terminal import TerminalCog
from tabs.raw_cogs.identify import IdentifyCog

class RawModule(ctk.CTkFrame):
    def __init__(self, parent, engine):
        ...
        lbl_index = ctk.CTkLabel(self.idx_frame, text="Diagnostic Index:", font=ctk.CTkFont(family="Segoe UI", size=13, weight="bold"))
        lbl_index.pack(anchor="w", padx=15, pady=(15, 10))

        cogs = ["1. Raw Pipeline Terminal", "2. Image Metadata Diagnostics"]
    ...
    def switch_view_box(self, index):
        ...
        if index == 0: TerminalCog(self.view_box, self.engine).grid(row=0, column=0, sticky="nsew")
        elif index == 1: IdentifyCog(self.view_box, self.engine).grid(row=0, column=0, sticky="nsew")
